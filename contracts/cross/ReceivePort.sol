...
abstract contract ReceivePort is IReceivePort {
    ...
    function getRoot(uint fromChainId, uint rootIndex) public view returns (bytes32) {
        return roots[fromChainId][rootIndex];
    }
    ...
    function verify(
        ...
        bytes32 leaf = keccak256(
            abi.encodePacked(msgHash, sender)
        );
        return _processProof(proof, leaf) == roots[fromChainId][rootIndex];
    }
