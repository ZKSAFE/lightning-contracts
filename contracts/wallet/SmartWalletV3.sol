...
contract SmartWalletV3 {
    ...
    function _doAtomCall(bytes calldata atomCallBytes) private {
        ...
        while(i < atomCallBytes.length) {
            ...
            bool hasValue = bytes1(atomCallBytes[i+20:i+21]) != 0x00;

            if (hasValue) {
                uint value = uint(bytes32(atomCallBytes[i+21:i+53]));
                uint len = uint(bytes32(atomCallBytes[i+53:i+85]));

                (bool success, bytes memory result) = to.call{value: value}(atomCallBytes[i+85:i+85+len]);
                if (!success) {
                    assembly {
                ...
                uint len = uint(bytes32(atomCallBytes[i+21:i+53]));

                (bool success, bytes memory result) = to.call{value: 0}(atomCallBytes[i+53:i+53+len]);
                if (!success) {
                    assembly {
                        revert(add(result, 32), mload(result))
                    }
