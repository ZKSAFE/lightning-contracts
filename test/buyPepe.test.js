...
const PEPE_TOKEN = new Token(ChainId, PEPE_CONTRACT_ADDRESS, 18, 'USDC', 'USD Coin')
...
describe('buyToken.test', function () {
    ...
    before(async function () {
        ...
        pepe = ERC20.attach(PEPE_CONTRACT_ADDRESS)
    })
    ...
    let amountIn
    let amountOut = m(1000000, tokenOut.decimals)
    let swapRoute
    it('quote', async function () {
        //USDC -> WETH
        let poolInfo0 = await getPoolInfo(USDC_TOKEN, WETH_TOKEN, FeeAmount.LOW)
        let pool0 = new Pool(
            USDC_TOKEN,
            WETH_TOKEN,
            FeeAmount.LOW,
            poolInfo0.sqrtPriceX96.toString(),
            poolInfo0.liquidity.toString(),
            poolInfo0.tick
        )
        
        //WETH -> PEPE
        let poolInfo1 = await getPoolInfo(WETH_TOKEN, PEPE_TOKEN, FeeAmount.HIGH)
        let pool1 = new Pool(
            WETH_TOKEN,
            PEPE_TOKEN,
            FeeAmount.HIGH,
    ...
    async function getPoolInfo(tokenA, tokenB, tradeFee) {
    ...
    async function print() {
        ...
        console.log('bundler usdc:', d(await usdc.balanceOf(bundler.address), USDC_TOKEN.decimals), 'weth:', d(await weth.balanceOf(bundler.address), 18), 'pepe:', d(await pepe.balanceOf(bundler.address), 18))
        console.log('wallet usdc:', d(await usdc.balanceOf(wallet.address), USDC_TOKEN.decimals), 'weth:', d(await weth.balanceOf(wallet.address), 18), 'pepe:', d(await pepe.balanceOf(wallet.address), 18))
