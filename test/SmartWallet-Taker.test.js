...
describe('SmartWallet-Taker-test', function () {
    ...
    it('quote', async function () {
        amountIn = await provider.getBalance(wallet.address)
    ...
    it('account1 atomSign', async function () {
        ...
        data = ERC.interface.encodeFunctionData('transfer(address,uint256)', [subBundler.address, m(0.5, USDC_TOKEN.decimals)])
        callArr.push({to, value, data})
    ...
    async function atomSign(signer, fromWallet, callArr) {
        ...
        let calldata = SmartWallet.interface.encodeFunctionData('atomSignCall', [toArr, valueArr, dataArr, deadline, []])
